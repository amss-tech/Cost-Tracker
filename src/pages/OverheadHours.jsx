...
export default function OverheadHours() {
  ...
  const totalCost = filtered.reduce((s, e) => s + ((e.rate || 0) * (e.hours || 0)), 0)
  ...
  function handleCSV() {
    const headers = ['Date', 'Employee', 'Job / Activity', 'Earn Code', 'Cost Code', 'Hours', 'Rate', 'Cost', 'Week Period', 'Status']
    const rows = filtered.map(e => [
      e.work_date, e.employee, e.job_name || '', e.earn_code,
      e.cost_code || '', e.hours,
      e.rate || '', e.rate ? parseFloat((e.hours * e.rate).toFixed(2)) : '',
      e.week_period || '', e.status,
    ])
    ...
    <>
      ...
      <div className="page">
        <div className="metric-grid" style={{ gridTemplateColumns: 'repeat(5,1fr)' }}>
          ...
          <div className="metric-card">
            <div className="metric-label">Total Cost (filtered)</div>
            <div className="metric-value" style={{ fontSize: 18 }}>{fmt.currency(totalCost)}</div>
          </div>
          <div className="metric-card">
        ...
        <div className="card"><div className="table-wrap">
          <table>
            <thead><tr>
              ...
              <th className="text-right">Hours</th><th className="text-right">Rate</th><th className="text-right">Cost</th><th>Week</th><th>Status</th>
            </tr></thead>
            <tbody>
              {filtered.length === 0
                ? <tr><td colSpan={10} style={{ textAlign: 'center', color: 'var(--color-text-3)', padding: 32 }}>No overhead entries found.</td></tr>
                : filtered.map(e => (
                  <tr key={e.id}>
            ...
            {filtered.length > 0 && (
              <tfoot>
                <tr style={{ background: 'var(--color-sidebar)', fontWeight: 600 }}>
                  ...
                  <td colSpan={1} />
                  <td className="text-right" style={{ padding: '8px 12px' }}>{fmt.currency(totalCost)}</td>
                  <td colSpan={2} />
                </tr>
              </tfoot>
            )}
